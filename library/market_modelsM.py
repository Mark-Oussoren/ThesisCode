...
class mean_rev_stock(bs_stock):
	...
	def generate_price(self, dt, St=None, *args, **kwargs):
		if St is None:
			St = self.price
		jump = 0
		self.Mp = np.random.poisson(self.LAMBDA * 2)
		# Still not sure what the point of Mp is here? Rename
...
class real_stock:
	def __init__(self, data, n_steps=60, data_freq=6, recycle=True, n_train=0, *args, **kwargs):
		"""
		:param data: stock data, pandas DataFrame of bid (float), bidSize (int), ask (float), askSize (int),
					 buyMO (int), sellMO (int), buySellImb (int), orderImb (int), spread (float)
		:param n_steps: # of time steps agent can act, int
	...
	def reset(self, training=True, *args, **kwargs):
		if (not training) and (self.n_train > 0):
	...
	def generate_price(self, dt, *args, **kwargs):
		"""
		:return: The bid price at a given data index divided by the initial price S0
	...
	def _scale(self, col, index, for_state=False, *args, **kwargs):
		"""
		:param col: name of one column in the limit order book dataframe, string
		:param index: asdf, int
		:param for_state: asdf, boolean
		:return: scaled index of some column in the LOB data - scale varies based on column
		"""
		if col == "bid" or col == "ask":
			if for_state:
				return (self.data[col].values[index] / (self.S0 - 1)) * 400
			else:
				return self.data[col].values[index] / self.S0
		elif col == "askSize" or col == "bidSize" or col == "buyMO":
			# TODO - why was this only returing zero beforehand?
			return self.data[col].values[index]
		elif col == "buySellImb":
			res = self.data[col].values[index]
			return res / max(self.data["buyMO"].values[index],
							 self.data["sellMO"].values[index]) - 0.5 * int(for_state)
		elif col == "orderImb":
			res = self.data[col].values[index]
			return res / max(self.data["bidSize"].values[index],
							 self.data["askSize"].values[index]) - 0.5 * int(for_state)
		elif col == "spread":
...
class lob_market(market):
	def __init__(self, stock_, n_hist_prices, lo_cap=10_000, perc_fee=0):
		"""
		:param stock_: Dataframe
		:param n_hist_prices:
		:param lo_cap:
		:param perc_fee:
		"""
		...
		self.reset_lo()
		# No permenant market impact
		self.b = 0
		self.lo_cap = lo_cap
		# For now, LOs can be made but not cancelled
		self.perc_fee = perc_fee # Fee charged for all LOs upon posting
		self.hist = {
			"bid" : [],
			"ask" : [],
			"askSize" : [],
	...
	def execute_lob(self, max_vol):
		# The use of max_vol is a shortcut but this doesn't affect the validity of the results
